...
> 後來又恢復為「Bar 區塊（`1804:13177`）仍在使用 `Spacing/sm(12)` 與 `Spacing/m(16)`」。
> **2026-08-01 重新查證，後面這個具體歸屬也站不住腳**：
> 對 `1804:13177` 整節點查詢 `get_variable_defs` 兩次，第一次回傳含這兩個變數，
> 第二次（相同節點、相同參數）不含。逐一查了節點底下全部 20 個非 iOS 子節點
> （App Bar Desktop / Mobile 各自的 User Info 與 icon、四種 nav_bar 的全部 variant、
> Bottom Bar、Search Bar 與其 icon），沒有一個帶有 `Spacing/sm(12)` 或
> `Spacing/m(16)`。這代表 `get_variable_defs` 對大節點的整節點查詢可能不是逐層窮舉，
> 而是有非決定性的取樣行為，不能拿它的「有」或「沒有」當唯一證據。
>
> **現狀是查無實據，不是「已修好」也不是「確認還在」** —— 上一版說「仍在使用」
> 已經超出目前查證能給出的把握。需要設計端直接在 Figma 裡選取 Bar 區塊的圖層，
> 用右側面板核對是否還有引用這兩個變數；本文件不再代為斷言歸屬。

程式端一律映射到主要命名（`--lds-spacing-s/m/l`），這件事不受此更正影響。
Figma 端若核對後仍有引用，修法是把剩下的引用改綁過去，再刪掉括號命名的那三個變數。

### A2. `Type Scale/body/L` 與 `body/L` 衝突

`Type Scale/body/L` = 14，`body/L` = 20。舊的 `Type Scale` 群組應移除。

**2026-08-01 查明歸屬**：`Type Scale/body/L` 目前唯一可查到的引用者是
**Status bar**（`1607:7869`，iOS 系統元件，時間／電量列），兩次查詢結果一致。
它屬於 [C 區](#c-尚未移植的-figma-元件)排除範圍，不影響程式碼；
但可以解釋這個舊群組還沒被清乾淨的原因 —— 只剩這一個孤兒引用它。
Figma 端要移除 `Type Scale` 群組前，記得先把 Status bar 這個引用改掉。
